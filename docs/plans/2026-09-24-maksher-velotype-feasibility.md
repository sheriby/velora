...
**目标：** 用可复现的实测结果判断 Velotype 的原生编辑器是否适合作为 maksher 选择性抽取的基础。

**架构：** maksher 的代码直接在当前工作区开发。Velotype 固定版本只在独立临时副本中构建和测量，不改动原始 `../velotype`。先验证原版性能与中文输入，再制定编辑部件抽取计划。
...
- 设计依据：[maksher 原生编辑器设计](2026-09-24-maksher-gpui-editor-design.md)。用户选择新建 maksher 的 GPUI 应用结构，一次性抽取 Velotype 的编辑代码，后续按需手工移植上游修复。
- 固定的 Velotype 来源提交为 `ed65977be94f2f2703037fcb8b6cbab2e7579571`。原始仓库在 `../velotype`，独立基线副本在 `/private/tmp/maksher-velotype-baseline`。
...
- 本阶段不把 Velotype 全仓复制进 maksher，不开发工作区功能，不恢复已删除的 Tauri 代码。性能或中文输入若未达到目标，先处理问题或重新评估方案。
...
2. 用 `git -C /private/tmp/maksher-velotype-baseline rev-parse HEAD` 核对固定提交；用 `git -C ../velotype status --short` 核对原始仓库未被修改。
...
1. 在当前 maksher 工作区运行 `node scripts/generate-fixtures.mjs /private/tmp/maksher-velotype-fixtures`。确认生成 1 MiB 与 10 MiB 两个 UTF-8 Markdown 文件；不要把大样本提交到 Git。
...
**文件：** 在独立基线副本临时修改 `src/main.rs`、`src/editor/render.rs`、`src/components/block/input.rs`；在 maksher 中新增 `docs/benchmarks/velotype-probe.patch` 并更新基线报告。
...
4. 在独立基线副本运行 `cargo build --release --locked`；把该副本的 `git diff` 输出保存为当前 maksher 仓库中的 `docs/benchmarks/velotype-probe.patch`。不要把探针直接合入原始 `../velotype` 或 maksher 正式应用。提交探针补丁与方法说明。
...
**文件：** 新建 `docs/plans/2026-09-24-maksher-editor-extraction-decision.md`。
