...
Velora 是以长篇 Markdown 写作为主的桌面编辑器。首版先发布 macOS；Windows 在下一版正式发布，首版只要求能构建内部安装包。默认使用富文本编辑，另有 Markdown 源码模式；在单根文件夹工作区中浏览 Markdown 和代码文件。初版不做 AI、Linux、终端、Git 集成、公式或 Mermaid。

首版功能包括文件树、多标签、最近工作区、文件名与内容搜索、文内查找、标题大纲、文件和文件夹的新建/重命名/移动/删除、代码编辑与语法高亮、图片复制到工作区目录并使用相对路径、自动保存、撤销与意外退出恢复。当前界面以文件树和内容为主，工作区入口与最近工作区位于 macOS 菜单栏，文件操作位于文件树右键菜单；可打开 docs/design/界面原型.html 查看布局参考。
