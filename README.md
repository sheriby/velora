...
当前代码的 `fastdev` 全量测试为 791 项通过、0 失败。macOS 已有历史 `.app` 与 `.pkg` 打包验证；本轮主题与排版改动尚未重新打包。最新布局可看[界面原型图](docs/design/界面原型-最新.png)，但当前远程图形会话无法获取真实原生窗口截图；Windows 实机运行和整机性能阈值仍待验收。
