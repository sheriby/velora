...
当前 `fastdev` 全量测试为 792 项通过、0 失败，另有 1 项手动性能诊断默认跳过。10 MiB 样本的构建与首次绘制约需 5.25 秒，仍略高于 5 秒目标。macOS `.app` 与 `.pkg` 的上一轮构建已完成包内容核对；本地 GPUI 修补尚未重新打包。最新布局可看[界面原型图](docs/design/界面原型-最新.png)，但当前远程图形会话无法获取真实原生窗口截图；Windows 实机运行和整机性能阈值仍待验收。
